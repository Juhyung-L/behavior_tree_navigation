...
void ProgressChecker::initialize(const rclcpp_lifecycle::LifecycleNode::WeakPtr& parent)
{
    ...
    nav2_util::declare_parameter_if_not_declared(
        node, name_ + ".minimum_distance", rclcpp::ParameterValue(0.5));
    nav2_util::declare_parameter_if_not_declared(
        node, name_ + ".movement_time_allowance", rclcpp::ParameterValue(5.0));
    
    minimum_distance_ = node->get_parameter(name_ + ".minimum_distance").as_double();
    movement_time_allowance_ 
        = rclcpp::Duration::from_seconds(node->get_parameter(name_ + ".movement_time_allowance").as_double());
}
